...
const toCanonicalSeniority = (value: string): "senior" | "manager" | "employee" | undefined => {
  const normalized = value.trim().toLowerCase().replace(/[^a-z0-9 /]/g, "");
  // Senior / Senior Management / Senior Staff
  if (
    ["senior", "senior management", "senior staff", "senior manager", "seniormanagement"].includes(normalized) ||
    normalized.startsWith("senior")
  ) return "senior";
  // Manager / Team Lead / Acting Manager / etc.
  if (
    ["manager", "manager team lead", "manager / team lead", "team lead", "teamlead", "lead", "acting manager"].includes(normalized) ||
    normalized.includes("manager") ||
    normalized.includes("team lead")
  ) return "manager";
  // Employee / Individual Contributor / Staff
  if (
    ["employee", "individual contributor", "staff", "associate", "analyst", "specialist", "officer", "coordinator", "engineer"].includes(normalized) ||
    normalized === "individual contributor"
  ) return "employee";
  return undefined;
};
...
const parseExcelRows = (filePath: string): TImportRow[] => {
  ...
  const HEADER_ALIASES: Record<string, string> = {
    ...
    grade: "seniorityLevel",
    employeegrade: "seniorityLevel",
  ...
  return rawRows.map((row) => {
    ...
    const seniorityRaw = normalizedRow.seniorityLevel || normalizedRow.seniority || normalizedRow.senioritylevel || "";
    ...
    return {
      ...
      seniorityLevel: seniorityRaw ? toCanonicalSeniority(seniorityRaw) : undefined,
    };
  });
};
...
const uploadEmployeeExcel = async (payload: {
  ...
  type ValidatedRow = {
    ...
    canonicalSeniorityLevel?: "senior" | "manager" | "employee";
  };
  ...
  for (let index = 0; index < rows.length; index++) {
    ...
    try {
      ...
      validRows.push({
        ...
        canonicalSeniorityLevel: row.seniorityLevel,
      });
    } catch (err: any) {
      summary.failed += 1;
      ...
      updateOne: {
        ...
        update: {
          $set: {
            ...
            ...(r.canonicalSeniorityLevel ? { seniorityLevel: r.canonicalSeniorityLevel } : {}),
            completed: false,
          },
          $setOnInsert: {
...
const getScannerSession = async (token: string) => {
  ...
  return {
    ...
    prefill: {
      ...
      seniorityLevel: invite.seniorityLevel ?? null,
    },
    seniorityRequired: !invite.seniorityLevel,
  };
};
...
const startSurveyByToken = async (payload: {
  ...
  // Use seniority stored in invite record (from Excel upload) if not provided by client
  const resolvedSeniority = payload.seniorityLevel ?? invite.seniorityLevel;

  if (!resolvedSeniority) {
    throw new AppError("Seniority level is required", httpStatus.BAD_REQUEST);
  }
